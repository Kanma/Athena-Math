...
void Quaternion::FromRotationMatrix(const Matrix3& kRot)
{
    ...
    if ( fTrace > 0.0 )
    {
        ...
        fRoot = MathUtils::Sqrt(fTrace + 1.0);  // 2w
        w = 0.5*fRoot;
    ...
    else
    {
        ...
        fRoot = MathUtils::Sqrt(kRot[i][i]-kRot[j][j]-kRot[k][k] + 1.0);
...
void Quaternion::FromAngleAxis(const Radian& rfAngle, const Vector3& rkAxis)
{
    ...
    Real fSin = MathUtils::Sin(fHalfAngle);
    w = MathUtils::Cos(fHalfAngle);
    x = fSin*rkAxis.x;
    y = fSin*rkAxis.y;
    z = fSin*rkAxis.z;
}
...
void Quaternion::ToAngleAxis(Radian& rfAngle, Vector3& rkAxis) const
{
    ...
    if ( fSqrLength > 0.0 )
    {
        rfAngle = 2.0*MathUtils::ACos(w);
        Real fInvLength = MathUtils::InvSqrt(fSqrLength);
...
Quaternion Quaternion::Exp() const
{
    ...
    Radian fAngle ( MathUtils::Sqrt(x*x+y*y+z*z) );
    Real fSin = MathUtils::Sin(fAngle);

    Quaternion kResult;
    kResult.w = MathUtils::Cos(fAngle);

    if ( MathUtils::Abs(fSin) >= SINE_CUTOFF )
    {
...
Quaternion Quaternion::Log() const
{
    ...
    {
        Radian fAngle ( MathUtils::ACos(w) );
        Real fSin = MathUtils::Sin(fAngle);
        if ( MathUtils::Abs(fSin) >= SINE_CUTOFF )
        {
...
{
    Real len = Norm();
    Real factor = 1.0f / MathUtils::Sqrt(len);
    *this = *this * factor;
    return len;
}
                             ...
                             bool shortestPath)
{
    ...
    if (MathUtils::Abs(fCos) < 1 - SINE_CUTOFF)
    {
        // Standard case (slerp)
        Real fSin = MathUtils::Sqrt(1 - MathUtils::Sqr(fCos));
        Radian fAngle = MathUtils::ATan2(fSin, fCos);
        Real fInvSin = 1.0f / fSin;
        Real fCoeff0 = MathUtils::Sin((1.0f - fT) * fAngle) * fInvSin;
        Real fCoeff1 = MathUtils::Sin(fT * fAngle) * fInvSin;
...
{
    Real fCos = rkP.Dot(rkQ);
    Radian fAngle ( MathUtils::ACos(fCos) );

    if ( MathUtils::Abs(fAngle.valueRadians()) < SINE_CUTOFF )
        return rkP;

    Real fSin = MathUtils::Sin(fAngle);
    Radian fPhase ( MathUtils::PI*iExtraSpins*fT );
    Real fInvSin = 1.0/fSin;
    Real fCoeff0 = MathUtils::Sin((1.0-fT)*fAngle - fPhase)*fInvSin;
    Real fCoeff1 = MathUtils::Sin(fT*fAngle + fPhase)*fInvSin;
